...
## 1. Устройство LLM (Large Language Model)

- LLM — это **нейросеть, обученная предсказывать следующий токен текста** на основе предыдущих.
- Основная архитектура — **трансформер**:
    - **Энкодер/декодер** (в большинстве генеративных моделей используется только декодер)
      - **Энкодер** = понимание контекста входного текста
      - **Декодер** = генерация текста на основе этого контекста  
    - **Self-attention** — механизм, который позволяет модели учитывать контекст всего предыдущего текста при выборе следующего токена
    - **Feed-forward слои** — нелинейные преобразования для генерации скрытых представлений
    - **Позиционное кодирование** — чтобы модель понимала порядок слов
- На вход модели подаются **токены** (слов или подслов), а на выходе — вероятности для каждого следующего токена.

**Токенизатор**:
- Разбивает текст на токены (например, слова, подслова или символы)
- Преобразует токены в числа (индексы в словаре модели)
- Позволяет модели работать с текстом как с числовыми последовательностями

---

## 2. Семплинг (Sampling)

- **Sampling** — метод генерации текста, когда следующий токен выбирается случайно согласно распределению вероятностей, предсказанному моделью.
- Основные виды:
    1. **Greedy decoding**
        - Выбирается **самый вероятный токен**
        - Низкая креативность, риск повторов
    2. **Temperature**
        - Управляет «остротой» распределения:
            - Малое T → детерминированный выбор
            - Большое T → более случайный выбор, креативный текст
    3. **Top-k sampling**
        - Рассматриваются только k самых вероятных токенов
        - Выбор случайного токена из этих k → контролируемая креативность
    4. **Top-p (nucleus) sampling**
        - Выбираются токены, суммарная вероятность которых ≥ p
        - Динамический размер словаря → гибкая генерация
- Комбинирование Top-k и Top-p позволяет найти баланс **связности и разнообразия текста**

---

## 3. Краткий принцип работы генерации текста

1. Токенизатор преобразует текст в последовательность токенов
2. Модель предсказывает вероятности для следующего токена
3. Применяется семплинг (Temperature, Top-k, Top-p)
4. Выбирается токен и добавляется к последовательности
5. Повторение шагов до завершения генерации

**Итог:**  
LLM — это вероятностная машина предсказаний токенов, где параметры семплинга управляют **стилем и креативностью** текста.
